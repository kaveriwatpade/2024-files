		...
		try {
			...
			Promise.all([findNodeDataSettings(userTimezoneSettings.companyId, queryParameters.nodeId)]).then(function (nodeResult) {
				try {
					...
					for (var i = 0; i < data.length; i++) {
						date = moment(data[i].reading_date, 'YYYY-MM-DD').format('DD-MM-YYYY');
						extractedValues = (null != data[i].fvalues && '' != data[i].fvalues) ? data[i].fvalues.split(',') : [];
...
exports.getCummulativeMonthlyData = function (request, cb) {
	...
	} catch (e) { console.log('monthly error', e) }
...
exports.getInstantaneousMonthlyData = function (request, cb) {
	...
	} catch (e) { console.log('monthly Instantaneous error', e) }
...
exports.getCummulativeYearlyData = function (request, cb) {
	...
	} catch (e) { console.log('Cummulative Yearly error', e) }
	Promise.all(promiseAry).then(function (results) {
		try {
			var data = results[0];
			var objData = { data: {} };
...
exports.getInstantaneousYearlyData = function (request, cb) {
	...
	} catch (e) { console.log('Instantaneous Yearly error', e) }
	Promise.all(promiseAry).then(function (results) {
		try {
			var data = results[0];
			var objData = { data: {} };
							...
							for (var parameterId in dataObj[nodeID]) {
								...
								for (var monthNumber in dataObj[nodeID][parameterId]) {
									...
									for (var samples = 0; samples < dataObj[nodeID][parameterId][monthNumber].length; samples++) {
										...
										avg = avg + parseInt(dataObj[nodeID][parameterId][monthNumber][samples][0]);
	...
	return new Promise((resolve, reject) => {
		report.findyearlytestReadingsByNode(queryParameters, function (err, data) {
			...
			try {
						...
						for (var parameterId in dataObj[nodeID]) {
							...
							for (var yearNumber in dataObj[nodeID][parameterId]) {
								...
								for (var samples = 0; samples < dataObj[nodeID][parameterId][yearNumber].length; samples++) {
									...
									avg = avg + parseInt(dataObj[nodeID][parameterId][yearNumber][samples][0]);
				...
				console.log('yearly exception', error)
			}
			return resolve(dataArray);
		})
	})
...
function findReadingsweeklytestByNode(queryParameters, parameterType) {
	return new Promise((resolve, reject) => {
		...
		report.findWeeklytestReadingsByNode(queryParameters, startdate, enddate, function (err, data) {
			...
			var xx = {}
			var a, s, e, sample = 0, count = 0, min, max;
			try {
				for (var i = 0; i < data.length; i++) {
					if (null != data[i].dvalues && '' != data[i].dvalues) {
						var date = data[i].reading_date;
						var weeknumber = momentTimezone(date, "YYYY-MM-DD").isoWeek();
						if (xx[data[i].fnode_id] == undefined) {
							xx[data[i].fnode_id] = {}
						}
						if (xx[data[i].fnode_id][data[i].fparameters] == undefined) {
							xx[data[i].fnode_id][data[i].fparameters] = {}
						}
						if (xx[data[i].fnode_id][data[i].fparameters][weeknumber] == undefined) {
							xx[data[i].fnode_id][data[i].fparameters][weeknumber] = []
						}
						var a = data[i].dvalues.split(',');
						xx[data[i].fnode_id][data[i].fparameters][weeknumber].push(a)
					}
				}
				var z = {}; var dataArray = [];
				if (0 == parameterType) {
					for (var nodeID in xx) {
						if (z[nodeID] == undefined) {
							z[nodeID] = {}
						}
						for (var parameterID in xx[nodeID]) {
							if (z[nodeID][parameterID] == undefined) {
								z[nodeID][parameterID] = {}
							}
							for (var weeknumber in xx[nodeID][parameterID]) {
								var object = {}
								if (z[nodeID][parameterID][weeknumber] == undefined) {
									z[nodeID][parameterID][weeknumber] = []
								}
								s = xx[nodeID][parameterID][weeknumber][xx[nodeID][parameterID][weeknumber].length - 1][0];
								e = xx[nodeID][parameterID][weeknumber][0][1];
								d = e - s;
								z[nodeID][parameterID][weeknumber].push(s, e, d)
								z[nodeID][parameterID][weeknumber] = z[nodeID][parameterID][weeknumber];
								for (var samples = 0; samples < xx[nodeID][parameterID][weeknumber].length; samples++) {
									count = parseInt(xx[nodeID][parameterID][weeknumber][samples][4]);
									count = count + count;
									/* *alternate way to cal avg of array elements */
									// let average = (array) => array.reduce((a, b) => a + b) / array.length;
									sample = (sample + parseFloat(xx[nodeID][parameterID][weeknumber][samples][3]));
								}
								sample = sample / xx[nodeID][parameterID][weeknumber].length
								z[nodeID][parameterID][weeknumber].push(sample, count)
								z[nodeID][parameterID][weeknumber] = z[nodeID][parameterID][weeknumber].toString();
								object['wnode_id'] = nodeID;
								object['wparameters'] = parameterID;
								object['wdate'] = parseInt(queryParameters.fromYear);
								object['wvalues'] = z[nodeID][parameterID];
							}
							dataArray.push(object)
						}
					}
						...
						for (var parameterID in xx[nodeID]) {
							...
							for (var weeknumber in xx[nodeID][parameterID]) {
								...
								for (var samples = 0; samples < xx[nodeID][parameterID][weeknumber].length; samples++) {
									...
									min = xx[nodeID][parameterID][weeknumber][samples][2];
									max = xx[nodeID][parameterID][weeknumber][samples][3];
									if (typeof a['min'] != 'undefined') {
										a['min'].push(min)
									}
									if (typeof a['max'] != 'undefined') {
									...
									avg = avg + parseInt(xx[nodeID][parameterID][weeknumber][samples][0]);
									count = parseInt(xx[nodeID][parameterID][weeknumber][samples][5]);
									count = count + count;
									sample = (sample + parseFloat(xx[nodeID][parameterID][weeknumber][samples][4]));
								}
								a['min'] = math.min(a['min'])
								a['max'] = math.max(a['max'])
								avg = avg / xx[nodeID][parameterID][weeknumber].length;
								sample = sample / xx[nodeID][parameterID][weeknumber].length
								z[nodeID][parameterID][weeknumber].push(avg, 0, a['min'], a['max'], sample, count)
								z[nodeID][parameterID][weeknumber] = z[nodeID][parameterID][weeknumber].toString();
								object['wnode_id'] = nodeID;
								object['wparameters'] = parameterID;
								object['wdate'] = parseInt(queryParameters.fromYear);
				...
				console.log('weekly exception ',e);
			}
			return resolve(dataArray);
		});
	});
