...
var ssh2Connection = function (req, res, existingData) {
		...
		try {
			...
			if (true == req.body.isActive) {
				...
				if ('alarm' == req.body.category) {
					...
					if (true == req.body.limitedNotification) {
						query = EquationQuery + '#window.length(2)' + commonSelect + '\'' + id + '\' as id, min(' + queryObj.query.group.rules[0].parameter.parameterId + ') as value1, max(' + queryObj.query.group.rules[0].parameter.parameterId + ') as value2 ,' +
				...
				/* if category is notification then add notification query */
					...
					if (req.session.passport.user.company_id != '00e24a07-0731-4712-ae0e-7af7bc536da6') {
						...
						messageString += "'" + sessionParameterObject[fieldName[1]]['display_name'] + "'," + paramData;
