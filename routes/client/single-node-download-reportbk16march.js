...
/* function applyDecimalUnit(x, numberFormat) {
  if ('' == x || null == x) {
    return '-';
  }
  else {
    var readingAry = x.split(' ');
    var value = readingAry[0] * 100 / 100;
    return value.toLocaleString(numberFormat, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + readingAry[1];
  }
} */

/* function applyUnitDecimalPipe(reading, defaultSymbol = '') {
  if ('' == reading || null == reading) {
    return '-';
  }
  else {
    let readingAry = reading.split(' ');
    let value = readingAry[0];
    let unit = ('' != readingAry[1]) ? readingAry[1] : defaultSymbol;
    return (value == 0) ? this.decimalPipe.transform(value, '1.3-3') + ' ' + unit : ('' == value || null == value || isNaN(value)) ? '-' : this.decimalPipe.transform(value, '1.3-3') + ' ' + unit;
  }
}
 */
