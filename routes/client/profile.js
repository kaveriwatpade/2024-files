...
router.post('/submitBasic', authenticationHelpers.isClientAuth, function (req, res) {
...
router.post('/submitSecurity', authenticationHelpers.isClientAuth, function (req, res) {
...
router.post('/submitRegional', authenticationHelpers.isClientAuth, function (req, res) {
	if (req.body.data.timezoneSetting == true) {
		req.checkBody("data.timezone", "User timezone is required.").notEmpty();
	}
	...
	req.getValidationResult().then(function (errors) {
		...
		profile.submitRegional(req.body, userId, function (err) {
			if (err) {
				res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
				return res;
			}
			req.session.passport.user.language = req.body.data.language;
			req.session.passport.user.timezone = req.body.data.timezone;
			req.session.passport.user.timezone_offset = req.body.data.offset;
			res.json({ "error": false, "reason": "Profile updated successfully." });
			res.end();
			return res;
		});
