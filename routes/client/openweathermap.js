...
OpenWeatherMapHelper.prototype.getDailyForecastByGeoCoordinates = function(latitude, longitude, callback) {
	request.get(`https://api.openweathermap.org/data/2.5/forecast/daily?lat=${parseFloat(latitude)}&lon=${parseFloat(longitude)}&APPID=${this.config.APPID}&units=${this.config.units}`,
	 (err, data)=>{
		sendResponse(err, data, callback);
	})
};
