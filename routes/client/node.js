...
// add by kaveri for applist

router.post('/getAppListforNdode', authenticationHelpers.isClientAuth, function (req, res) {
  node.getAppListforNdode(req.body, function (err, result) {
...
router.post('/getSingleNodeDeleteData', authenticationHelpers.isClientAuth, function (req, res) {
  node.getSingleNodeDeleteData(req.body, function (err, result) {
    // console.log(err, 'error!!!!!!!!');
    if (err) {
      res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
      return res;
...
//added by kaveri for save benchmark
router.post('/submitBenchmark', authenticationHelpers.isClientAuth, function (req, res) {
  console.log(req.body.action, '-=================');

  if (req.body.action == 'add') {
    node.submitBenchmark(req.body, req.session.passport.user, function (err) {
      // console.log(req.body,'req.body');
      if (err) {
        res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
        return res;
      }
      res.json({ "error": false, "reason": "Benchmark added successfully." });
      res.end();
      return res;
    });
  }
  if (req.body.action == 'edit') {
    node.updatetBenchmark(req.body, req.session.passport.user, function (err) {
      console.log(req.body,'req.body11111111');
      if (err) {
        // console.log(err, 'error@@@@@@@@@@@');
        res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
        return res;
      }
      res.json({ "error": false, "reason": "Benchmark update successfully." });
      res.end();
      return res;
    });
  }
...
router.post('/nodeDataBenchmark', authenticationHelpers.isClientAuth, function (req, res) {
  node.nodeDataBenchmark(req.body, function (err, result) {
    // console.log(req.body, '(req.body');
    console.log(err, 'error!!!!!!!!!!!!!!!');
    if (err) {
      res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
...
//displaying benchmark list
router.post('/benchmarkList', authenticationHelpers.isClientAuth, function (req, res) {
  node.benchmarkList(req.session.passport.user.company_id, function (err, result) {
...
//delete benchmark
router.post('/deleteBenchmark', authenticationHelpers.isClientAuth, function (req, res) {
  // console.log(req.body, 'req.body req.body');
  node.deleteBenchmark(req.body, function (err) {
    console.log(err, 'error@@@@@@@@@@@@@@@@');
    if (err) {
      res.json({ "error": true, "reason": "Error: Something went wrong. Please try again!" });
      return res;
    }
    res.json({ "error": false, "reason": "Benchmark deleted successfully." });
    res.end();
    return res;
  });
});
