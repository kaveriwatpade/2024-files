...
function generateSingleNodeComparisonReportExcelFile(req, session, download) {
  console.log('in generateSingleNodeComparisonReportExcelFile');
  return new Promise((resolve, reject) => {
    try {
      var promiseAry = [];
      var columnAry = JSON.parse(req.body.columns);
      ...
      Promise.all(promiseAry).then(promiseResult => {
        ...
        var rowAry = [];
        var rowDataAry = [];
// console.log(promiseResult[0]['data'],'0000000');
// console.log(promiseResult[1]['data'],'1111111');
        for (var date in promiseResult[0]['data']) {
          var dataObj = {};
          dataObj['srno'] = dateIndex;
...
function generateComparisonReportExcelFile(req, session, download) {
  return new Promise((resolve, reject) => {
    try {
      ...
      var interval = req.body.interval;
      var company_name = session['company_name'];
      // console.log(postDataObj[0],'-====-=-=-=-=-=-=-=-=-=',postDataObj[1]);
      promiseAry.push(getPostdataReadings(req, postDataObj[0]));
      ...
      Promise.all(promiseAry).then(promiseResult => {
        ...
        for (var currentperiod in promiseResult[0]['data']) {
          ...
          for (var nodeUniqueIdParamId in promiseResult[0]['data'][currentperiod]) {
            ...
            // console.log(promiseResult[0]['data'],'promiseResult[0]');
...
function generateComparisonReportPDFFile(req, session, download) {
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log(postDataObj[0],'1111');
      console.log(postDataObj[1],'2222');
      promiseAry.push(getPostdataReadings(req, postDataObj[0]));
      promiseAry.push(getPostdataReadings(req, postDataObj[1]));
      ...
      Promise.all(promiseAry).then(promiseResult => {
        var dateIndex = 1;
        var rowAry = [];
        // console.log( promiseResult[0]['data'],' promiseResult[0]["data"]');
        // console.log( promiseResult[1]['data'],' promiseResult[1]["data"]');
        for (var currentperiod in promiseResult[0]['data']) {
          var dataObj = {};
          dataObj['srno'] = dateIndex;
          dataObj['currentperiod'] = currentperiod;
...
function getPostdataReadings(req, postDataObj) {
  ...
  // console.log(postDataObj,'postdatobj');
  return new Promise((resolve, reject) => {
    try {
      var isInstantaneous = parseInt(req.body.isinstantaneous);
      // console.log(req.body,'req.body.isinstantaneous');
      ...
      switch (interval) {
        ...
        case 'instantaneousmonthly': common.getInstantaneousMonthlyData(req, function (error, data) {
          // console.log(data,'-------');
          if (error) {
            // console.log(error,'error111111111111111111');
            return reject(error);
          }
          return resolve(data);
        });
            ...
            for (var date in promiseResult[0]['data']) {
              ...
              for (var nodeUniqueIdParamId in promiseResult[1]['data'][date]) {
                ...
                if (parameters[nodeId[1]].parameter_type != 0) {
                  ...
                  // dataObj['r' + nodeUniqueIdParamId] = promiseResult[1]['data'][date][nodeUniqueIdParamId]['unitReading']
...
function generateSingleNodeComparisionReportCsvFile(req, session, download) {
  console.log('in generateSingleNodeComparisionReportCsvFile');
  return new Promise((resolve, reject) => {
    try {
      var postDataObj = req.body.postData;
      postDataObj[0]['reportType'] = (req.body.interval == 'tenmin') ? 'Ten Min Report' : req.body['reportType'];
...
var rowAry3 = [];
        ...
        for (var currentperiod in promiseResult[0]['data']) {
          ...
          //console.log('rowAry !!! Value : ' + rowAry1)
          dateIndex++;
        }
      ...
      //  var rowAry1 = [];
        ...
        if(rowAry1.length == rowAry2.length){
          for(var x = 0; x < rowAry2.length ;x++){
            rowAry3.push(rowAry1[x] +','+ rowAry2[x]);
           // console.log('rowAry3: ' + rowAry3)
          }
        }else if(rowAry1.length < rowAry2.length ){
          for(var x = 0; x < rowAry2.length ;x++){
            if(x > (rowAry1.length-1)){
              rowAry3.push( rowAry2[x]);
            }else{
              rowAry3.push( rowAry2[x] + ',' + rowAry1[x]);
            }
            
           // console.log('rowAry3: ' + rowAry3)
          }
        }else if(rowAry1.length > rowAry2.length){
          for(var x = 0; x < rowAry1.length ;x++){
            rowAry3.push(rowAry1[x] +','+ rowAry2[x]);
           // console.log('rowAry3: ' + rowAry3)
          }
        }
