					...
					for (var i=0; i<data.length; i++) {
						...
						for(var key in parameterValues){
							...
							if(objNonSortData[nodeTime]){
								...
								if(1 || (d['lowestReading'] = 0.000 && e['lowestReading'] - s['lowestReading'] != d)){
						...
						if(objWeeks['slot'][wYear]){
							for(var key in objWeeks['slot'][wYear]){
								if(dataObject[key]){
									...
									objNonSortData[weekDate][data[i].wnode_id+'_'+data[i].wparameters] = {min:mathFormat(extractedValues[2],defaultReadingAndUnit['dataUnitObj'][data[i].wnode_id+'_'+data[i].wparameters],defaultReadingAndUnit['smallestUnitVal']),max:mathFormat(extractedValues[3],defaultReadingAndUnit['dataUnitObj'][data[i].wnode_id+'_'+data[i].wparameters],defaultReadingAndUnit['smallestUnitVal']),average:mathFormat(extractedValues[0],defaultReadingAndUnit['dataUnitObj'][data[i].wnode_id+'_'+data[i].wparameters],defaultReadingAndUnit['smallestUnitVal']),sampling:parseFloat(extractedValues[4]),count:parseFloat(extractedValues[5]),standardDeviation:parseFloat(extractedValues[1])};
								}
							}
						}
					}
  ...
  var objPreviousValues = {};
	for(var key in objNonSortData){
		...
		for(var node in objNonSortData[key]){
          ...
          //console.log(defaultReadingAndUnit);
					objNonSortData[previousKey][node]['difference'] = (null !== objNonSortData[previousKey][node]['difference']['unitReading']) ? mathEval(objNonSortData[previousKey][node]['difference']['unitReading'] +' + ('+objNonSortData[key][node]['startReading']['unitReading']+' - '+objNonSortData[previousKey][node]['endReading']['unitReading']+')',defaultReadingAndUnit['dataUnitObj'][node],defaultReadingAndUnit['smallestUnitVal']) : (mathEval('0 + ('+objNonSortData[key][node]['startReading']['unitReading']+' - '+ objNonSortData[previousKey][node]['endReading']['unitReading']+')',defaultReadingAndUnit['dataUnitObj'][node],defaultReadingAndUnit['smallestUnitVal']));
...
// function testSortCummulativeData(defaultReadingAndUnit,objNonSortData,requestObject,lastRecordDate){
// 	var previousKey = '';
// 	var objReadings = {};
// 	var objPreviousValues = {};
// 	for(var key in objNonSortData){
...
// 				if('undefined' !== typeof objPreviousValues[node]){
// 					previousKey = objPreviousValues[node];
// 					objNonSortData[previousKey][node]['difference'] = (null !== objNonSortData[previousKey][node]['difference']['unitReading']) ? testMathEval(objNonSortData[previousKey][node]['difference']['unitReading'] +' + ('+objNonSortData[key][node]['startReading']['unitReading']+' - '+objNonSortData[previousKey][node]['endReading']['unitReading']+')',defaultReadingAndUnit['dataUnitObj'][node],defaultReadingAndUnit['smallestUnitVal']) : (testMathEval('0 + ('+objNonSortData[key][node]['startReading']['unitReading']+' - '+ objNonSortData[previousKey][node]['endReading']['unitReading']+')',defaultReadingAndUnit['dataUnitObj'][node],defaultReadingAndUnit['smallestUnitVal']));
// 					objNonSortData[previousKey][node]['endReading'] = objNonSortData[key][node]['startReading'];
// 				}
// 				objPreviousValues[node] = '';
// 				objPreviousValues[node] = key;
...
mathFormat = function(readingValue,dataUnitObj,smallestUnit){
  ...
  var actualValue = math.to(math.unit(readingValue,dataUnitObj['dataUnit'][0]),dataUnitObj['dataUnit'][0]);//Convert to unit
	//Convert to visualization defined unit. If not defined then set to auto
...
// testMathFormat = function(readingValue,dataUnitObj,smallestUnit){
// 	if(null === readingValue || readingValue == 'null' || 'undefined' == typeof readingValue){
// 		return {unitReading:null,lowestUnitReading:null,lowestReading:null,reading:null,unit:''};
// 	}
// 	if('' == dataUnitObj['dataUnit'][0]){ //If data incoming unit not defined then return value
...
// 		return {unitReading:readingValue+' ',lowestUnitReading:readingValue+' ',lowestReading:parseFloat(readingValue),reading:parseFloat(readingValue),unit:''};
// 	}
// 	var truncated = math.floor(readingValue * 1000) / 1000;
// 	readingValue = truncated;
// 	var actualValue = math.to(math.unit(readingValue,dataUnitObj['dataUnit'][0]),dataUnitObj['dataUnit'][0]);//Convert to unit
// 	//Convert to visualization defined unit. If not defined then set to auto
// 	var convertedValue = (dataUnitObj['autoUnit'] === true || '' == dataUnitObj['dataUnit'][1]) ? math.format(actualValue,{notation:"auto",precision:3,exponential:{lower:1e-50,upper:1e+50}}) : math.format(actualValue.to(dataUnitObj['dataUnit'][1]),{notation:"fixed",precision:3,exponential:{lower:1e-50,upper:1e+50}});
// 	convertedValue = convertedValue.replace(/[()]/g, '');
...
// 	if('undefined' != typeof unitPrefixObj['nameObj'][dataUnitObj['dataUnit'][1]] && smallestUnit){
// 		if(unitPrefixObj['nameObj'][dataUnitObj['dataUnit'][1]]['value'] > smallestUnit && 'undefined' != typeof unitPrefixObj['valueObj'][dataUnitObj['baseUnit']][smallestUnit]){
// 			lowestConvertedValue = (dataUnitObj['autoUnit'] === true || '' == dataUnitObj['dataUnit'][1]) ? math.format(actualValue,{notation:"fixed",precision:5,exponential:{lower:1e-50,upper:1e+50}}) : math.format(actualValue.to(unitPrefixObj['valueObj'][dataUnitObj['baseUnit']][smallestUnit]),{notation:"fixed",precision:5,exponential:{lower:1e-50,upper:1e+50}});
// 			lowestConvertedValue = lowestConvertedValue.replace(/[()]/g, '');
// 		}
// 	}
// 	var convertedValueAry = convertedValue.split(' ');
...
/*comment by ankita and deepa
mathEval = function(expression,dataUnitObj,smallestUnit) {
  console.log(expression,dataUnitObj,smallestUnit)
  try{
    if(null === expression || expression == 'null' || 'undefined' == typeof expression){//If expression blank
      return {unitReading:null,lowestUnitReading:null,lowestReading:null,reading:null,unit:''};
    }
...
// testMathEval = function(expression,dataUnitObj,smallestUnit) {
// 	if(null === expression || expression == 'null' || 'undefined' == typeof expression){//If expression blank
// 		return {unitReading:null,lowestUnitReading:null,lowestReading:null,reading:null,unit:''};
// 	}
// 	if('' == dataUnitObj['dataUnit'][0]){ //If data incoming unit not defined then return value
