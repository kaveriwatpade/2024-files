...
var ksql = require(__base + 'models').Ksql;
					...
					if (1 == request.body.isinstantaneous) {
						...
						for (var i = startIndex; i >= 0; i--) {
							...
							request.body.parameterId.forEach(function (parameter, index) {
								...
								console.log('dbReading', dbReading);
								...
								// console.log(nodeId+"==========",JSON.stringify(readings[nodeId+'_'+parameter]));
							});
						}
						for (var key in readings) {
...
function getKsqlAPIData(reqObj, queryParameters) {
	return new Promise((resolve, reject) => {
		switch (reqObj['interval']) {
			case 'fifteen':
				ksql.getFifteen(queryParameters, reqObj['currentDate'], function (err, data) {
					if (err) {
						return reject(err);
					}
					return resolve(data);
			...
			case 'hourly':
				ksql.getHourly(queryParameters, reqObj['currentDate'], function (err, data) {
					if (err) {
						return reject(err);
					}
					return resolve(data);
...
function findHourlyReadingsByNodeForDaily(interval, queryParameters) {
	...
	return new Promise((resolve, reject) => {
		report.findHourlyReadingsByNodeForDaily(queryParameters, function (err, data) {
			if (err) {
				console.log('findHourlyReadingsByNodeForDaily', err);
				return reject(err);
			}
			return resolve(data);
		});
