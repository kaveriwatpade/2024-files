...
router.post('/partnerCompanyUserList', authenticationHelpers.isAdminAuth, function(req, res){
	if(req.body.companyId){
		user.partnerCompanyWiseUsersList(req.session.passport.user.partner_id,req.body.companyId,function(err, result){
			if (err) { 
				res.json({"error" : true,"reason": "Error: Something went wrong. Please try again!"}); 
				return res;
			}
			if (!result || !result['partnerCompanyUser']) {  
				res.json({"error" : true,"reason": "No records found."}); 
				return res; 
			}
			result['partnerCompanyUser'].forEach(function (user,k) {
				if(user.user_id == req.session.passport.user.user_id){
					result['partnerCompanyUser'].splice(k,1);
				}
			});
		...
		user.partnerCompanyUserList(req.session.passport.user.partner_id,function(err, result){
			if (err) { 
				res.json({"error" : true,"reason": "Error: Something went wrong. Please try again!"}); 
				return res;
			}
...
router.post('/companyList', authenticationHelpers.isAdminAuth, function(req, res){
...
router.post('/submitUser', authenticationHelpers.isClientAuth, function (req, res) {
	...
	commonfunctionmodel.checkIfPermissionExists(req.session.passport.user, permissionId, function (data) {
		...
		// req.checkBody("data.language", "Language must be select.").notEmpty();
		var errorMsg = '';
...
router.post('/userData', authenticationHelpers.isAdminAuth, function (req, res) {
...
router.post('/userUpdate', authenticationHelpers.isAdminAuth, function(req, res){
	req.checkBody("data.firstName", "First Name is required.").notEmpty();
	req.checkBody("data.lastName", "Last Name is required.").notEmpty();
	req.checkBody("data.email", "Email address is not valid.").isEmail();
	req.checkBody("data.mobile", "Mobile number must be digit.").isNumeric();
...
router.post('/deleteUser', authenticationHelpers.isAdminAuth, function (req, res) {
...
router.post('/changePwd', authenticationHelpers.isAdminAuth, function (req, res) {
...
router.post('/userAlreadyExists', authenticationHelpers.isAdminAuth, function (req, res) {
	...
	user.emailOrMobileAlreadyExists(req.body.email, 'email_address', function (err, emailResult) {
		...
		if (emailResult && 'undefined' != typeof emailResult) {
			if (emailResult['partners']) {
				res.json({ "error": true, "reason": "This email is already associated with partner!" });
				return res;
			}
			...
			if (emailResult['companies'].hasOwnProperty(req.body.companyId)) {
		...
		if (!emailResult || 'undefined' == typeof emailResult) {
			user.emailOrMobileAlreadyExists(req.body.mobile, 'mobile_number', function (err, mobileResult) {
				if (mobileResult && 'undefined' != typeof mobileResult) {
					if (mobileResult['partners']) {
						res.json({ "error": true, "reason": "This mobile is already associated with partner!" });
						return res;
					}
...
router.post('/updateCompanyIdForExistingUser', authenticationHelpers.isAdminAuth, function (req, res) {
	user.updateCompanyIdForExistingUser(req.body, function (err, result) {
		...
		try {
			...
			htmlObj['body'] = '<p>You are receiving this email because you have been added as MARC user in ' + req.body['companyName'] + ' company by ' + req.session.passport.user.firstname + ' ' + req.session.passport.user.lastname + '.</p>';
...
router.post('/addUser', authenticationHelpers.isAdminAuth, function (req, res) {
	req.checkBody("data.firstName", "First Name is required.").notEmpty();
	req.checkBody("data.lastName", "Last Name is required.").notEmpty();
	req.checkBody("data.email", "Email address is not valid.").isEmail();
	req.checkBody("data.mobile", "Mobile number must be digit.").isNumeric();
