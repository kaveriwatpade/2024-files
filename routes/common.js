...
//Added by Arjun : Tenant Billing -Start ///
var nodemailer = require("nodemailer");
global.mailSettingDetails = [];
global.mailSettingDetails1 = [];
...
//Added by Arjun : Tenant Billing - End ///
...
exports.getCummulativeFifteenMinutesData = function (request, cb) {
	...
	/****created by kaveri - get data for selected slots -start */
	var start = request.body.fromDate.split(' ')[1].slice(5)
	var end = request.body.toDate.split(' ')[1].slice(5)
	var s = request.body.fTime + start
	var e = request.body.tTime + end
	// console.log(s, 'ssssssssssssss', e);

	var fromDate = request.body.fromDate.split(' ')[0] + ' ' + s
	var toDate = request.body.toDate.split(' ')[0] + ' ' + e
	var fromDateTime = fromDate.split('+');//Split timezone and datetime
	var toDateTime = toDate.split('+');//Split timezone and datetime

	/****created by kaveri - get data for selected slots */
	...
	/* Ksql chages for Time-Slice cron handling current date data (if shift ended on tomorrow)  by MAYUR */
	var reqObj = request.fromDifferentApi != 'Time-slice' && hour >= 6 ? { interval: request.body.interval, currentDate: currentDate } : { interval: request.body.interval, currentDate: currentDate, currentDateTime: momentTimezone(currentDateObj).format('YYYY-MM-DD') };
	/* Ksql chages for Time-Slice cron handling current date data (if shift ended on tomorrow) by MAYUR */
				...
				try {
					...
					for (var i = 0; i < data.length; i++) {
						...
						for (var key in parameterValues) {
							...
							// /** write code here------------kaveri */
							// if (request.body.fTime <= time[0] && request.body.tTime >= time[1]) {
							// 	kvTime = time;
							// 		nodeTime = date + ' ' + kvTime[0] + ' - ' + kvTime[1];
					...
					for (var i = 0; i < data.length; i++) {
						...
						for (var key in parameterValues) {
							...
							if (objNonSortData[nodeTime]) {
								...
								// console.log(objNonSortData[nodeTime][data[i].fnode_id + '_' + data[i].fparameters], ']]]]]]');
							}
							// }
						}
					}
					objData['data'] = sortInstantaneousData(defaultReadingAndUnit, objNonSortData, '', nodeTime);
...
exports.getCummulativeDailyData = function (request, cb) {
	...
	queryParameters.isinstantaneous = request.body.isinstantaneous; // Added by Arjun
			...
			Promise.all([findNodeDataSettings(userTimezoneSettings.companyId, queryParameters.nodeId)]).then(function (nodeResult) {
				try {
					...
					for (var i = 0; i < data.length; i++) {
						...
						var month = fromMonth;
						startingIndex = monthIndex;
						for (; startingIndex <= 1200; startingIndex++) {
							if (toMonth < month) {
								break;
							}
							if (dataObject[startingIndex]) {
...
// 	var reqObj = { interval: request.body.interval };
// 	var arrMonthsYear = createMonthsYear(queryParameters.fromDate);
// 	promiseAry.push(findReadingsmonthlytestByNode(queryParameters, arrMonthsYear, 1));
			...
			Promise.all([findNodeDataSettings(userTimezoneSettings.companyId, queryParameters.nodeName)]).then(function (nodeResult) {
				try {
					...
					for (var i = startIndex; i >= 0; i--) {
						...
						// let TesttimeStamp = readingDateTime.format('HH:mm:ss');
						// if(TesttimeStamp >= FTimr && TesttimeStamp <= tTime)
					...
					// console.log(readings,'readingsreadings');
...
function createIntervalDataArray1(fromDateTime, toDateTime, userTimezone, intervalInMinutes, ftime, ttime) {
	console.log(fromDateTime, 'fromDateTime')
	console.log(toDateTime, 'toDateTime')
		...
		switch (reqObj['interval']) {
			case 'fifteen':
				ksqlTest.getFifteen(queryParameters, reqObj['currentDate'], function (err, data) {
					...
					if (reqObj['currentDateTime']) {
						ksqlTest.getFifteen(queryParameters, reqObj['currentDateTime'], function (err1, data1) {
							if (err1) {
								return reject(err1);
							}
							data1 = lodash.union(data, data1)
			...
			case 'hourly':
				...
				break;
			case 'daily':
				ksqlTest.getDaily(queryParameters, reqObj['currentDate'], function (err, data) {
					if (err) {
						return reject(err);
					}
					return resolve(data);
	...
	for (var key in objNonSortData) {
		...
		for (var node in objNonSortData[key]) {
			...
			if (null != objNonSortData[key][node]['startReading']['unitReading']) {
				if ('undefined' !== typeof objPreviousValues[node] && !Number.isNaN(objNonSortData[key][node]['startReading']['reading'])) {
...
function sortInstantaneousData1(defaultReadingAndUnit, objNonSortData, lastRecordDate, nodeTime) {
	...
	if (!lastRecordDate) {
		console.log('222');
		console.log(nodeTime, 'nodeTimenodeTime');

		for (var key in objNonSortData) {
			if (1 == Object.keys(objNonSortData[key]).length) {
				timestamp = objNonSortData[nodeTime]['timestamp'];

				console.log(objNonSortData[nodeTime], 'objNonSortData');
				console.log(key, 'timestamp');
				objNonSortData[nodeTime] = lodash.cloneDeep(defaultReadingAndUnit['defaultReadingObject']);
				objNonSortData[nodeTime]['timestamp'] = timestamp;
			}
		}
...
function sortInstantaneousData(defaultReadingAndUnit, objNonSortData, lastRecordDate) {
	// console.log(objNonSortData,'objNonSortData');
	if (lastRecordDate) {
		var objReadings = {};
		for (var key in objNonSortData) {
			if (1 == Object.keys(objNonSortData[key]).length) {
...
function getUnitPrefix() {
	if (Object.keys(unitPrefixObj).length == 0) {
		...
		if (!math.type.Unit.isValuelessUnit('TON')) {
			math.createUnit({
				TON: {
					base: {
						key: "TON"
		...
		if (!math.type.Unit.isValuelessUnit('CELM10')) {
			math.createUnit({
				CELM10: {
					base: {
						key: "CELM10"
		...
		if (!math.type.Unit.isValuelessUnit('CEL')) {
			math.createUnit({
				CEL: {
					base: {
						key: "CEL"
...
exports.getCummulativeTimeSliceFifteenMinutesData = function (request, cb) {
	...
	/* Ksql chages for solar cron by MAYUR */
	var promiseAry = [];
	var ksqlAPICall = 0, dbCall = 0, ksqlAPICall1 = 0;
	...
	if (momentTimezone.utc(toDateTime[0]).format('YYYY-MM-DD') == currentDate || currentDateObj.diff(atoDateTimeObject, 'd') <= 0) {
		...
		promiseAry.push(getKsqlAPIData(reqObj, queryParameters));
		if (hour >= 0 && hour <= 6) {
			currentDate = momentTimezone(currentDateObj).format('YYYY-MM-DD');
			reqObj = { interval: request.body.interval, currentDate: currentDate };
			if (momentTimezone.utc(toDateTime[0]).format('YYYY-MM-DD') == currentDate || currentDateObj.diff(atoDateTimeObject, 'd') <= 0) {
				ksqlAPICall1 = 1;
				promiseAry.push(getKsqlAPIData(reqObj, queryParameters));
			}
	...
	Promise.all(promiseAry).then(function (results) {
		try {
			...
			else if (ksqlAPICall && ksqlAPICall1) {
				ksqlData = lodash.union(results[0], results[1]);
			}
			else if (ksqlAPICall) {
				ksqlData = results[0];
			}
...
exports.getCummulativeAvg = function (request, cb) {

	dailyData = request.body.dailyData.data;
	dataKey = request.body.nodeId[0] + '_' + request.body.parameterId[0];

	avgData = {}
	avgData.difference = {};
	avgData.endReading = {};
	avgData.startReading = {};

	avgData.acceleration = 0;
	avgData.count = 0;
	avgData.sampling = 0;
	avgData.speed = 0;

	avgData.startReading.lowestReading = 0;
	avgData.startReading.lowestUnitReading = 0.0;
	avgData.startReading.reading = 0;
	avgData.startReading.unit;
	avgData.startReading.unitReading = 0.0;

	avgData.endReading.lowestReading = 0;
	avgData.endReading.lowestUnitReading = 0.0;
	avgData.endReading.reading = 0;
	avgData.endReading.unit;
	avgData.endReading.unitReading = 0.0;

	avgData.difference.lowestReading = 0;
	avgData.difference.lowestUnitReading = 0.0;
	avgData.difference.reading = 0;
	avgData.difference.unit;
	avgData.difference.unitReading = 0.0;
	...
	for (const [Datakey, Datavalue] of Object.entries(dailyData)) {

		if ((Datavalue[dataKey].startReading.unitReading != null) && (Datavalue[dataKey].endReading.unitReading != null) && (Datavalue[dataKey].difference.unitReading != null)) {
			...
			if (counter == 0) {
				...
				avgData.startReading.lowestReading = Datavalue[dataKey].startReading.lowestReading;
				avgData.startReading.reading = Datavalue[dataKey].startReading.reading;
				avgData.startReading.lowestUnitReading = Datavalue[dataKey].startReading.lowestUnitReading;
				avgData.startReading.unitReading = Datavalue[dataKey].startReading.unitReading;
				avgData.startReading.unit = Datavalue[dataKey].startReading.unit;

				counter++;
				// avgData.startReading.lowestReading = JSON.parse(avgData.startReading.lowestReading.toFixed(0));
				// avgData.startReading.reading = JSON.parse(avgData.startReading.reading.toFixed(0));

			}

			avgData.endReading.lowestReading = Datavalue[dataKey].endReading.lowestReading;
			avgData.endReading.reading = Datavalue[dataKey].endReading.reading;
			avgData.endReading.lowestUnitReading = Datavalue[dataKey].endReading.lowestUnitReading;
			avgData.endReading.unitReading = Datavalue[dataKey].endReading.unitReading;
			avgData.endReading.unit = Datavalue[dataKey].endReading.unit;

		}

	}

	avgData.difference.unit = avgData.endReading.unit
	avgData.difference.lowestReading = JSON.parse(avgData.endReading.lowestReading - avgData.startReading.lowestReading);
	avgData.difference.reading = JSON.parse(avgData.endReading.reading - avgData.startReading.reading);
	avgData.difference.lowestUnitReading = (avgData.difference.lowestReading.toFixed(3) + ' ' + avgData.difference.unit);
	avgData.difference.unitReading = (avgData.difference.reading.toFixed(3) + ' ' + avgData.difference.unit);
...
exports.getInstantaneousAvg = function (request, cb) {
	...
	avgData1.sampling = 0;
	avgData1.count = 0;
	avgData1.standardDeviation = 0;

	avgData1.min.lowestReading = 0;
	avgData1.min.lowestUnitReading = 0.0;
	avgData1.min.reading = 0;
	avgData1.min.unit;
	avgData1.min.unitReading = 0.0;

	avgData1.max.lowestReading = 0;
	avgData1.max.lowestUnitReading = 0.0;
	avgData1.max.reading = 0;
	avgData1.max.unit;
	avgData1.max.unitReading = 0.0;

	avgData1.average.lowestReading = 0;
	avgData1.average.lowestUnitReading = 0.0;
	avgData1.average.reading = 0;
	avgData1.average.unit;
	avgData1.average.unitReading = 0.0;
	...
	for (const [Datakey, Datavalue] of Object.entries(dailyData)) {
		...
		if ((Datavalue[dataKey].min.unitReading != null) && (Datavalue[dataKey].max.unitReading != null) && (Datavalue[dataKey].average.unitReading != null)) {


			if (counter == 0) {
				setDate = Datakey;

				avgData1.min.lowestReading = Datavalue[dataKey].min.lowestReading;
				avgData1.min.reading = Datavalue[dataKey].min.reading;
				avgData1.min.unit = Datavalue[dataKey].min.unit;

				avgData1.max.lowestReading = Datavalue[dataKey].max.lowestReading;
				avgData1.max.reading = Datavalue[dataKey].max.reading;
				avgData1.max.unit = Datavalue[dataKey].max.unit;

			}

			if (avgData1.min.lowestReading > Datavalue[dataKey].min.lowestReading) {
			...
			if (avgData1.max.lowestReading < Datavalue[dataKey].max.lowestReading) {
			...
			avgData1.sampling = avgData1.sampling + Datavalue[dataKey].sampling;

			avgData1.average.lowestReading = avgData1.average.lowestReading + JSON.parse(Datavalue[dataKey].average.lowestReading);
			avgData1.average.reading = avgData1.average.reading + JSON.parse(Datavalue[dataKey].average.reading);
			avgData1.average.unit = Datavalue[dataKey].average.unit;
	...
	avgData1.sampling = JSON.parse((avgData1.sampling / counter).toFixed(3));

	// avgData1.min.lowestReading = JSON.parse((avgData1.min.lowestReading / counter).toFixed(3));
	// avgData1.min.reading = JSON.parse((avgData1.min.reading / counter).toFixed(3));
	avgData1.min.lowestUnitReading = avgData1.min.lowestReading + ' ' + avgData1.min.unit;
	avgData1.min.unitReading = avgData1.min.reading + ' ' + avgData1.min.unit;

	// avgData1.max.lowestReading = JSON.parse((avgData1.max.lowestReading / counter).toFixed(3));
	// avgData1.max.reading = JSON.parse((avgData1.max.reading / counter).toFixed(3));
	avgData1.max.lowestUnitReading = avgData1.max.lowestReading + ' ' + avgData1.max.unit;
	avgData1.max.unitReading = avgData1.max.reading + ' ' + avgData1.max.unit;

	avgData1.average.lowestReading = JSON.parse((avgData1.average.lowestReading / counter).toFixed(3));
	avgData1.average.reading = JSON.parse((avgData1.average.reading / counter).toFixed(3));
	avgData1.average.lowestUnitReading = avgData1.average.lowestReading + ' ' + avgData1.average.unit;
	avgData1.average.unitReading = avgData1.average.reading + ' ' + avgData1.average.unit;
...
exports.sendMail = function (request, cb) {
	params = { query: "SELECT * FROM dashboard_new" };
	general.dbSelect(params, function (err, result) {
		if (err != null) {
			console.log(err, "emailTest line 84");
			sendMailFun(request);
		} else {
			array = result;
			console.log(array);

			request.mails.map((item, index) => {
				var check = array.find((obj) => obj.dashboard_name == item);
				console.log(check);
				if (check != undefined) {
					request.mails.splice(index, 1);
				}
			});
			console.log(request.mails);
			sendMailFun(request);
		}
	});
};

function sendMailFun(request) {
	console.log("Cron server listening on port " + request.string);
	var htmlObj = {};
	htmlObj["user"] = "";
	htmlObj["body"] = request.string;
	htmlObj["imagePath"] = marcconfig.ANGULARBASEPATH + "/assets/images";
	htmlObj["basePath"] = marcconfig.ANGULARBASEPATH;

	console.log(request.filePath, "htmlObjhtmlObjhtmlObjhtmlObjhtmlObj");

	ejs.renderFile(
		__base + marcconfig.EMAILTEMPLATEPATH + "/emailTemplateBySopan.ejs",
		htmlObj,
		function (err, html) {
			var mailOptions = {
				from: "marccloud.live<noreply@goesl.co>",
				to: request.mails,
				replyTo: "marketing@goesl.co",
			...
			console.log(mailOptions, "mailOptionsmailOptionsmailOptionsmailOptions");
			...
			mailSettingDetails.sendMail(mailOptions, function (err, result) {
				if (err != null) {
					...
					mailSettingDetails1.sendMail(mailOptions, function (err, result) {
						if (err != null) {
							console.log(`err here - `, err);
							console.log(
								`==============================================================================`
