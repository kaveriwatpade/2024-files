...
exports.getCummulativeFifteenMinutesData = function(request,cb){
	...
	if(momentTimezone.utc(toDateTime[0]).format('YYYY-MM-DD') == currentDate || currentDateObj.diff(toDateTimeObject,'d') == 0){
		sparkAPICall = 1;
		promiseAry.push(getSparkAPIData(reqObj,queryParameters));
	}
	Promise.all(promiseAry).then(function (results){	
		try{
			var sparkData = [],dbData = [];
			if(dbCall && sparkAPICall){
				dbData = results[0];
				sparkData = results[1];
			}
			else if(sparkAPICall){
				sparkData = results[0];
			}
			else if(dbCall){
				dbData = results[0];
			}
      var data = lodash.union(sparkData,dbData);
			var objData = {data:{}};
      if(!data || 0 == data.length) { return cb(null, objData); }
...
exports.getInstantaneousFifteenMinutesData = function(request,cb){
	...
	if(momentTimezone.utc(toDateTime[0]).format('YYYY-MM-DD') == currentDate){
		sparkAPICall = 1;
		promiseAry.push(getSparkAPIData(reqObj,queryParameters));
	}
	Promise.all(promiseAry).then(function (results){
...
exports.realTimeData = function(request,cb){
  try{
    ...
    var nodeDataSettingObj = ('undefined' != typeof request.session.passport.user.nodeObj[request.body.meter] && null != request.session.passport.user.nodeObj[request.body.meter]) ? JSON.parse(request.session.passport.user.nodeObj[request.body.meter]) : '';
