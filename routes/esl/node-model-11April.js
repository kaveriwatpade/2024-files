...
router.post('/submitNodeModel', authenticationHelpers.isEslAuth, function (req, res) {
  ...
  req.getValidationResult().then(function (errors) {
    ...
    parameter['cummulative'].forEach((element, i) => {
      ...
      // parameterMapping[element] = 'C' + (parameter['instantaneous'].length + i);
    });
      ...
      nodemodel.updateNodeModel(req.body, req.session.passport.user.user_id, JSON.stringify(parameterMapping), function (err) {
        ...
        logic.getNodeModelNodes(req.body.id, function (err, nodeResult) {
          ...
          for (var nodeIndex = 0; nodeIndex < nodeResult.length; nodeIndex++) {
            var nodeModelDataSettingsObj = { queryLength: cellComObj['queryLength'], autoUnit: false, indexing: {}, equations: {}, incomingDataUnit: {}, displayDataUnit: {}, caption: {} };
            ...
            nodeObj[nodeResult[nodeIndex]['node_id']] = { nodeUniqueId: nodeResult[nodeIndex]['node_unique_id'], companyId: nodeResult[nodeIndex]['company_id'], dataSettings: JSON.stringify(nodeDataSettingsObj) };
